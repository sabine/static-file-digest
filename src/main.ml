let run output maybe_root_dir exts silent =
  ...
  in
  let digest_map = match maybe_root_dir with
    | None -> log "must specify a directory!\n"; Digest_map.empty
    | Some root_dir ->
      let _ = exts in
      let _ = output in
...
let () =
  let dir =
    Arg.(
      value & pos ~rev:true 0 (some string) None
      & info [] ~docv:"DIRECTORY"
          ~doc:"Directory.")
  in
  let output =
  ...
  let info =
    let doc =
      "Compute hash digest for every file in a directory - outputs a standalone OCaml module."
    in
    let man =
      [
        `S "BUGS";
        `P "Email bug reports to <sabine@tarides.com>.";
      ]
    in
    Cmd.info "static-file-digest" ~version:"%%VERSION%%" ~doc ~man
  in
